...
export function ReverbControls({
  ...
  return (
    ...
    >
      ...
      {/* Stack-memory bus (palimpsest playback) */}
      <p style={{ fontSize: fs(10), fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: theme.colors.ink2, paddingTop: 4, borderTop: `1px solid ${theme.colors.border}` }}>stack memory bus</p>
      <div className="grid grid-cols-3 gap-x-0.5 gap-y-2">
        <Slider label="Rev"   min={0} max={1}   step={0.01} value={layerMix}   onChange={onLayerMixChange}   format={(v) => `${Math.round(v * 100)}%`} hue={150} title={ttMix} />
        <Slider label="Room"  min={0} max={1}   step={0.01} value={layerSize}  onChange={onLayerSizeChange}  format={(v) => `${Math.round(v * 100)}%`} hue={150} />
        <Slider label="Sat"   min={0} max={0.8} step={0.01} value={layerDrive} onChange={onLayerDriveChange} format={(v) => `${Math.round((v / 0.8) * 100)}%`} hue={150} title={ttSat} />
      </div>
      <p className="leading-snug px-0.5" style={{ fontSize: fs(10), color: theme.colors.ink3 }}>
        stacked memory lives in the right-hand tape panel — press <kbd style={{ padding: "0 3px", borderRadius: 2, border: `1px solid ${theme.colors.borderMid}`, fontFamily: theme.font.mono, fontSize: fs(9) }}>R</kbd> to record passes.
      </p>
    </div>
  );
}
